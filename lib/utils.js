var markposts = (function(){
	...
	var merge_tags = function(callback){
		var file, filePag,
			handlerFile = function (file, type){
						fs.writeFile(file, JSON.stringify(tagsTmp[key]), function (err) {
							if (err){
	      ...
	      if (tagsTmp.hasOwnProperty(key)) {
			...
			if (config.pagination.enable === true){
				...
				for(i;i<paginas;i+=1){
					...
					handlerFile(filePag, 'Pagination ');
				}
			}
	      }
	    }//FOR
	...
	var create_tags = function(post, callback){
		var tags = post.meta.tags,
			c;
		var name = config.folder.dist.posts+'/'+slugify(post.meta.title.toLowerCase())+'.json';
			post.meta.url=name;
