var markposts = (function(){
	...
	var generateFiles = function(posts, cb){
		log.info('\nCreate json post\'s files');
		log.info('-----------------\n');
		async.each(posts,function(post, callback){	
			if (isEmpty(post.meta)){
				...
				create_post(post, function(err){
					...
					create_tags(post, function(err){
						...
						moveToProccess(post.filename+post.filenameExtension);
						callback();
					});
				});		
			}
			...
			if (err){
				...
				merge_tags(function(err){
					if (err) return cb('Error when merging the tags files');
					cb(null);	
				});
			}
		});
	};
	var moveToProccess = function(filename){
		var path = process.cwd() +'/'+ config.folder.articles+'/'+filename;
		var newPath = process.cwd() +'/'+ config.folder.process+'/'+filename;
	...
	var merge_tags = function(callback){
		...
		for (var key in tagsTmp) {
	      if (tagsTmp.hasOwnProperty(key)) {
			...
			if (config.pagination.enable === true){
				var pages =1,i=0, meta = {};
				pages = Math.ceil(tagsTmp[key].length / config.pagination.size );
				for(i;i<pages;i+=1){
		...
		if (config.pagination.enable === true){
			var pages =1,i=0, meta = {};
			pages = Math.ceil(totalPost.length / config.pagination.size );
			for(i;i<pages;i+=1){
				meta.total_pages = pages;
				meta.current_page = i+1;
	...
	var create_post = function(post, callback){
		var content = {},
		name = config.folder.dist.posts+'/'+slugify(post.meta.title.toLowerCase());
		post.meta.url=name;
				...
				if(err){
					...
					async.parallel({
						...
						articles: function(callback){
							fs.mkdir(proyectPath+'/'+config.folder.articles,function(err){
								if (err){
						...
						process: function(callback){
							fs.mkdir(proyectPath+'/'+config.folder.process,function(err){
								if (err){
			...
			createifnotexit(dist, function(err){
				...
				function(cb){
					...
					mm.parseDirectory(process.cwd() +'/'+ config.folder.articles, function(err, posts){
						generateFiles(posts, function(err){
